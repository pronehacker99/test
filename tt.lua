...
AutoPlaceEggsGroupBox:AddDropdown('EggToPlace', {
    Values = {},
    Default = nil,
    Multi = false,
    Text = 'Select Egg to Place',
    Tooltip = 'Select the egg you want to automatically place.',
    Searchable = true,
    MenuOpened = function()
        RefreshEggModels()
    end
...
local function RefreshEggModels()
    ...
    Options.EggToPlace:SetValues(EggList)

    if #EggList > 0 then
        Library:Notify("Found " .. #EggList .. " eggs in your backpack.")
    else
        Library:Notify("No eggs found in your backpack.")
    end
end
...
Toggles.EnableAutoPlaceEggs:OnChanged(function(value)
    if value then
        ...
        task.spawn(function()
            ...
            local delayTime = 1 -- seconds between egg placements

            local selectedEggName = Options.EggToPlace.Value
            ...
            -- Find the plot owned by the player
            ...
            -- Get random position inside plot bounds
        ...
        unequipItem() -- Unequip the egg when disabled
    end
end)
