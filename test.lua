...
AutoPlaceEggsGroupBox:AddToggle('EnableAutoPlaceEggs', {
    ...
    Default = false,
    Tooltip = 'Automatically places the selected egg in your plot.'
})

AutoPlaceEggsGroupBox:AddDropdown('AutoPlaceEggType', {
    Values = {"Click to Refresh"},
...
-- Function to refresh the egg list
function RefreshEggList()
    local eggModels = game:GetService("ReplicatedStorage").Assets.Models.EggModels:GetChildren()
    local eggNames = {}
    ...
    if #eggNames > 0 then
        ...
        Library:Notify("Updated egg list!", 1)
    else
        Library:Notify("No egg models found!", 2)
    end
end

-- Auto Place Egg Logic
local autoPlaceEggsActive = false
Toggles.EnableAutoPlaceEggs:OnChanged(function(value)
    autoPlaceEggsActive = value
    if value then
        task.spawn(function()
            local selectedEgg = Options.AutoPlaceEggType.Value
            if not selectedEgg or selectedEgg == "Click to Refresh" then
                Library:Notify("Please select an egg to place.", 3)
                Toggles.EnableAutoPlaceEggs:SetValue(false)
                return
            end

            if not equipItem(selectedEgg) then
                Library:Notify("Could not find or equip " .. selectedEgg .. ". Disabling.", 3)
                Toggles.EnableAutoPlaceEggs:SetValue(false)
                return
            end

            Library:Notify("Auto Place Eggs Enabled!", 2)
            ...
            local function randomPositionInPlot(plot)
                if plot:FindFirstChild("Base") then
                    ...
                    return Vector3.new(x, pos.Y + 1, z) -- Added +1 to Y to avoid placing inside the floor
                end
                return nil
            end

            while autoPlaceEggsActive do
                local myPlot = getMyPlot()
                if myPlot then
                    local pos = randomPositionInPlot(myPlot)
                    if pos then
                        local eggType = Options.AutoPlaceEggType.Value
                        PetEggService:FireServer("CreateEgg", pos, eggType)
                    end
                else
                    Library:Notify("Could not find your plot. Disabling Auto Place Eggs.", 3)
                    Toggles.EnableAutoPlaceEggs:SetValue(false)
                    break
                end
                task.wait(delayTime)
        ...
        Library:Notify("Auto Place Eggs Disabled!", 2)
        unequipItem()
    end
end)
