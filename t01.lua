...
-- =================================================================
-- AUTO PLACE EGGS (Added by Cline)
-- =================================================================
...
AutoPlaceEggsGroupBox:AddDropdown('AutoPlaceEggSelection', {
    Values = {"Open to Refresh"},
    Default = nil,
    Multi = false,
    Text = 'Select Egg to Place',
    Tooltip = 'Select an egg from your backpack to automatically place.',
    Searchable = true,
    MenuOpened = function()
        RefreshPlayerEggs()
    end,
...
-- Input for delay
AutoPlaceEggsGroupBox:AddInput('AutoPlaceEggDelay', {
    Default = '1',
    Numeric = true,
    Finished = true,
    Text = 'Placement Delay (seconds)',
    Tooltip = 'Set the delay between placing each egg.',
})
...
AutoPlaceEggsGroupBox:AddToggle('EnableAutoPlaceEgg', {
    ...
    Default = false,
    Tooltip = 'Automatically equips and places the selected egg on your plot.'
})

-- Function to get eggs from the player's backpack
function RefreshPlayerEggs()
    ...
    local eggItems = {}
    local foundEggNames = {} -- To avoid duplicates

    -- Get the list of possible egg base names from ReplicatedStorage
    local EggModels = game:GetService("ReplicatedStorage").Assets.Models.EggModels
    local possibleEggBaseNames = {}
    for _, eggModel in ipairs(EggModels:GetChildren()) do
        table.insert(possibleEggBaseNames, eggModel.Name)
    end

    for _, item in ipairs(backpack:GetChildren()) do
        if item:IsA("Tool") then
            for _, baseName in ipairs(possibleEggBaseNames) do
                -- Check if the backpack item name starts with a known egg base name
                if item.Name:match("^" .. baseName) and not foundEggNames[baseName] then
                    table.insert(eggItems, baseName)
                    foundEggNames[baseName] = true -- Mark as found
                    break -- Move to the next backpack item
                end
            end
        end
    end
    ...
    if #eggItems > 0 then
        Options.AutoPlaceEggSelection:SetValues(eggItems)
        Library:Notify('Found ' .. #eggItems .. ' egg types in your backpack.')
    else
        Library:Notify('No placeable eggs found in backpack.')
        Options.AutoPlaceEggSelection:SetValues({}) -- Clear the dropdown
    end
end

-- Main logic for auto-placing eggs
...
Toggles.EnableAutoPlaceEgg:OnChanged(function(value)
    ...
    if value then
        task.spawn(function()
            ...
            if not selectedEggName or selectedEggName == "" then
                Library:Notify("No egg selected. Disabling Auto Place.", 3)
                Toggles.EnableAutoPlaceEgg:SetValue(false)
                return
            end

            -- Equip the selected egg before starting the loop
            if not equipItem(selectedEggName) then
                Library:Notify("Could not find or equip '" .. selectedEggName .. "'. Disabling.", 3)
                Toggles.EnableAutoPlaceEgg:SetValue(false)
                return
            end
            ...
            -- Farm bounds (as provided by user)
            ...
            while autoPlaceEggActive do
                local delayTime = tonumber(Options.AutoPlaceEggDelay.Value) or 1
        ...
        -- Unequip any held item when disabling the feature
        unequipItem()
    end
end)
