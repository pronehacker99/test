-- Auto Place Selected Pet Egg (with fuzzy equip + auto plot)
-- Drop in your executor
...
-- Fuzzy tool finder: matches "Common Egg", "Common Egg x905", etc.
local function findToolByBaseName(baseName)
    local pattern = "^" .. escapePattern(baseName) .. "%s*x?%d*$" -- allows " x###" or nothing
...
-- Robust equip (replicates to server)
...
-- Find player's plot using multiple heuristics
...
-- Pick the biggest base part in the plot (or one named "Base")
...
-- Random world position on top of the base (handles rotation)
local function randomPointOnBase(basePart)
    local halfX = basePart.Size.X * 0.45 -- margin to avoid edges
    ...
    local world = (basePart.CFrame * CFrame.new(lx, topY, lz)).Position
    return Vector3.new(world.X, world.Y, world.Z)
end
...
-- Populate eggs from ReplicatedStorage.Assets.Models.EggModels
...
local function placeOnce()
    ...
    -- Equip matching tool (handles "Common Egg x905", etc.)
