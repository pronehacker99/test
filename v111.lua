...
-- Failsafe: destroy loading screen if initialization takes too long
...
Toggles.EnableAutoReroll:OnChanged(function(value)
    if value then
        ...
        local function send_hook(mutation, success)
            ...
            local thumb_url = "https://raw.githubusercontent.com/pronehacker99/test/refs/heads/main/logo.png" -- Default avatar
            local http_service = game:GetService("HttpService")
            ...
            if success then
                embed = {
                    ...
                    footer = { text = "PH99 Hub | Auto Reroll", icon_url = "https://raw.githubusercontent.com/pronehacker99/test/refs/heads/main/logo.png" },
            ...
            local data = {
                ...
                avatar_url = "https://raw.githubusercontent.com/pronehacker99/test/refs/heads/main/logo.png",
                embeds = { embed }
            }
...
-- AUTO HATCH EGGS
-- =================================================================
local AutoHatchGroupBox = MiscTab:AddLeftGroupbox('Auto Hatch Eggs', 'egg')
AutoHatchGroupBox:AddDivider()

AutoHatchGroupBox:AddInput('HatchDelay', {
    Default = '0.1',
    Numeric = true,
    Finished = true,
    Text = 'Hatch Delay (seconds)',
    Tooltip = 'Set the delay between each hatch attempt.'
})

AutoHatchGroupBox:AddToggle('EnableAutoHatch', {
    Text = 'Enable Auto Hatch',
    Default = false,
    Tooltip = 'Automatically hatches any eggs found in the workspace.'
})

local autoHatchActive = false
Toggles.EnableAutoHatch:OnChanged(function(value)
    autoHatchActive = value
    if value then
        Library:Notify('Auto Hatch Enabled!')
        task.spawn(function()
            ...
            while autoHatchActive do
                local workspace = game:GetService("Workspace")
                -- Search for eggs in the workspace
                for _, egg in pairs(workspace:GetDescendants()) do
                    if not autoHatchActive then break end
                    if egg.Name:find("PetEgg") and egg:IsA("Model") then
                        PetEggService:FireServer("HatchPet", egg)
                    end
                end
                local delay = tonumber(Options.HatchDelay.Value) or 0.1
                task.wait(delay)
            end
        end)
    else
        Library:Notify('Auto Hatch Disabled!')
    end
end)
...
AutoPlaceEggGroupBox:AddDropdown('AutoPlaceEggSelection', {
    Values = {"CommonEgg"}, -- Default, will be refreshed
    Default = "CommonEgg",
    Multi = false,
...
-- Input for the delay
AutoPlaceEggGroupBox:AddInput('AutoPlaceEggDelay', {
    Default = '0.1',
    Numeric = true,
    Finished = true,
    Text = 'Placement Delay (s)',
    Tooltip = 'Set the delay between each egg placement.',
})

-- Toggle to enable/disable
AutoPlaceEggGroupBox:AddToggle('EnableAutoPlaceEgg', {
    Text = 'Enable Auto Place',
    Default = false,
    Tooltip = 'Automatically places the selected egg on your farm.'
})

local autoPlaceEggActive = false

-- Helper functions from autoplaceegg.lua, adapted for this script
local function getFarmBoundsForEggPlacer()
    local Players = game:GetService("Players")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
local function raycastToGroundForEggPlacer(farm, x, yStart, z)
    local Workspace = game:GetService("Workspace")
...
local function randomPositionForEggPlacer()
	local bounds = getFarmBoundsForEggPlacer()
	...
	local y = raycastToGroundForEggPlacer(bounds.farm, x, bounds.center.Y, z)
	return Vector3.new(x, y, z)
end

local function toNiceEggNamesForEggPlacer(name)
...
local function equipEggFuzzyForEggPlacer(target)
    local Players = game:GetService("Players")
    local localPlayer = Players.LocalPlayer
        ...
        for _, tool in ipairs(container:GetChildren()) do
            if tool:IsA("Tool") then
                ...
                if typeof(toolName) == "string" then
                    ...
                    for _, variant in ipairs(toNiceEggNamesForEggPlacer(target)) do
...
Toggles.EnableAutoPlaceEgg:OnChanged(function(value)
    ...
    if value then
        Library:Notify('Auto Place Egg Enabled!')
        task.spawn(function()
            local ReplicatedStorage = game:GetService("ReplicatedStorage")
            local PetEggService = ReplicatedStorage:WaitForChild("GameEvents"):WaitForChild("PetEggService")
            
            while autoPlaceEggActive do
                local delay = tonumber(Options.AutoPlaceEggDelay.Value) or 0.1
                local selectedEggName = Options.AutoPlaceEggSelection.Value
                
                if not selectedEggName then
                    Library:Notify("No egg selected for placement.", 2)
                    task.wait(1)
                else
                    equipEggFuzzyForEggPlacer(selectedEggName)
                    local pos = randomPositionForEggPlacer()
                    if pos then
        ...
        Library:Notify('Auto Place Egg Disabled!')
    end
end)
...
do
    ...
    local function createBillboard(adornee)
        ...
        -- Lift above the fruit by half of its size if possible
        local offsetY = 2
        if adornee and adornee:IsA('BasePart') then
            offsetY = math.max(2, adornee.Size.Y * 0.75)
        end
    ...
    local function findFirstBasePart(inst)
        ...
        -- Prefer parts within the current container (fruit) first
        for _, d in ipairs(inst:GetDescendants()) do
            if d:IsA('BasePart') then return d end
        end
        -- Fallback to nearest ancestor model
    ...
    local function deduceType(nv)
        -- If the NumberValue sits under a child model/part with fruit name, prefer that
        local nameCandidate = nv.Parent and nv.Parent.Name
        if nameCandidate and nameCandidate ~= '' then
            return normalizeFruitName(nameCandidate)
        end
        local m = nv:FindFirstAncestorOfClass('Model')
        while m and m.Parent and m.Parent:IsA('Model') do m = m.Parent end
        local name = m and m.Name or nameCandidate or 'Unknown'
...
-- =================================================================
-- =================================================================
-- AUTO SHOVEL (integrated from autoshovel.lua)
-- =================================================================
local AutoShovelGroupBox = PlantsTab:AddRightGroupbox('Auto Shovel', 'shovel')
AutoShovelGroupBox:AddDivider()

AutoShovelGroupBox:AddDropdown('AutoShovelFruitType', {
	Values = { 'All' },
	Default = 'All',
	Multi = false,
	Text = 'Fruit Type',
	Tooltip = 'Only shovel fruits of this type.'
})

AutoShovelGroupBox:AddInput('AutoShovelThreshold', {
	Default = '3',
	Numeric = true,
	Finished = true,
	Text = 'Threshold (kg)',
	Tooltip = 'Delete fruits with weight ≤ this value.'
})

AutoShovelGroupBox:AddToggle('AutoShovelOnlyMyFarm', {
	Text = 'Only My Farm',
	Default = true,
	Tooltip = 'Restrict the shovel to your own farm.'
})

AutoShovelGroupBox:AddToggle('AutoShovelEnable', {
	Text = 'Enable Auto Shovel',
	Default = false,
	Tooltip = 'Automatically delete small fruits that meet the criteria.'
})
...
do
	...
	local function isInMyFarm(inst)
		if not (Toggles.AutoShovelOnlyMyFarm and Toggles.AutoShovelOnlyMyFarm.Value) then return true end
		if not myFarm or not myFarm.Parent then return true end
		return inst:IsDescendantOf(myFarm)
	end
	...
	local function getModelPivotPosition(m)
	...
	local function findAncestorFolderByName(inst, folderName)
	...
	local function getModelExtentsMagnitude(m)
	...
	local function isProbablyTreeOrStructure(m)
	...
	local function getFruitModelFromWeight(weightValue)
		...
		local candidatesGrowable = {}
		local candidatesWhitelist = {}
		local candidatesSafe = {}

		local weightPos = nil
		local weightPart = findFirstBasePart(weightValue.Parent)
		if weightPart then weightPos = weightPart.Position end

		local FRUIT_NAME_WHITELIST = {
		...
		local function pickSmallest(list)
		...
		local function pickClosestSmall(list)
	...
	local function getRemoveItemRemote()
	...
	local function autoShovelUnequipAll()
		local char = Players.LocalPlayer.Character or Players.LocalPlayer.CharacterAdded:Wait()
		local hum = char and char:FindFirstChildOfClass('Humanoid')
		if hum then hum:UnequipTools() end
	end

	local function autoShovelEquipShovel()
		local char = Players.LocalPlayer.Character or Players.LocalPlayer.CharacterAdded:Wait()
	...
	local function tryDeleteFruit(weightValue, removeItemRemote, deleteRemote)
	...
	local function deduceFruitType(weightValue)
		local fruitsFolder = findAncestorFolderByName(weightValue, 'Fruits')
		if fruitsFolder then
			for _, child in ipairs(fruitsFolder:GetChildren()) do
				if child:IsA('Model') and weightValue:IsDescendantOf(child) then
	...
	local function updateDropdownValues()
		...
		for t,_ in pairs(knownTypes) do if t ~= 'All' then table.insert(values, t) end end
		table.sort(values)
		if Options.AutoShovelFruitType then Options.AutoShovelFruitType:SetValues(values) end
	end
	...
	if Options.AutoShovelFruitType then
		Options.AutoShovelFruitType:OnMenuOpened(function()
			collectKnownTypes()
		end)
	end
	...
	local function runLoop()
		...
		while running and (Toggles.AutoShovelEnable and Toggles.AutoShovelEnable.Value) do
			if not (removeItemRemote or deleteRemote) then
				removeItemRemote = getRemoveItemRemote()
				deleteRemote = getDeleteRemote()
				if not (removeItemRemote or deleteRemote) then
					Library:Notify('Auto Shovel: Removal RemoteEvent not found. Retrying...', 2)
					task.wait(0.5)
					continue
				end
			end
			autoShovelEquipShovel()
			local selectedType = (Options.AutoShovelFruitType and Options.AutoShovelFruitType.Value) or 'All'
			local threshold = tonumber(Options.AutoShovelThreshold and Options.AutoShovelThreshold.Value) or 3
			refreshMyFarm()
			for _, inst in ipairs(Workspace:GetDescendants()) do
				if not running or not (Toggles.AutoShovelEnable and Toggles.AutoShovelEnable.Value) then break end
				if inst:IsA('NumberValue') and inst.Name == 'Weight' and findAncestorFolderByName(inst, 'Fruits') then
					if isInMyFarm(inst) then
						local t = deduceFruitType(inst)
						knownTypes[t] = true
						if selectedType == 'All' or t == selectedType then
	...
	Toggles.AutoShovelEnable:OnChanged(function(v)
		if v then
			Library:Notify('Auto Shovel enabled!')
			if not Options.AutoShovelThreshold.Value or Options.AutoShovelThreshold.Value == '' then
				Options.AutoShovelThreshold:SetValue('3')
			end
			collectKnownTypes()
			task.spawn(runLoop)
		else
			Library:Notify('Auto Shovel disabled!')
			running = false
		end
	end)

	Toggles.AutoShovelOnlyMyFarm:OnChanged(function()
		if running then
			refreshMyFarm()
			collectKnownTypes()
		end
